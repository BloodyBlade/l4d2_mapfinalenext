...
{
	GameData hGamedata = new GameData("l4d2_changelevel");
	if(hGamedata == null) SetFailState("Failed to load \"l4d2_changelevel.txt\" gamedata.");
	
	StartPrepSDKCall(SDKCall_Raw);
	if(!PrepSDKCall_SetFromConf(hGamedata, SDKConf_Signature, "CDirector::OnChangeChapterVote")) SetFailState("Error finding the 'CDirector::OnChangeChapterVote' signature.");
	PrepSDKCall_AddParameter(SDKType_String, SDKPass_Pointer);
	hDirectorChangeLevel = EndPrepSDKCall();
	if(hDirectorChangeLevel == null) SetFailState("Unable to prep SDKCall 'CDirector::OnChangeChapterVote'");
	TheDirector = GameConfGetAddress(hGamedata, "CDirector");
	if(TheDirector == Address_Null) SetFailState("Unable to get 'CDirector' Address");

	StartPrepSDKCall(SDKCall_Raw);
	if(!PrepSDKCall_SetFromConf(hGamedata, SDKConf_Signature, "CDirector::ClearTeamScores")) SetFailState("Error finding the 'CDirector::ClearTeamScores' signature.");
	PrepSDKCall_AddParameter(SDKType_Bool, SDKPass_Plain);
	hDirectorClearTeamScores = EndPrepSDKCall();
	if(hDirectorClearTeamScores == null) SetFailState("Unable to prep SDKCall 'CDirector::ClearTeamScores'");
...
{
	char sMapName[PLATFORM_MAX_PATH], temp[2];
...
int L4D2_ChangeLevelNV(Handle plugin, int numParams)
{
	...
	bool bResetScores = true;
	if(numParams >= 2) bResetScores = view_as<bool>(GetNativeCell(2));
